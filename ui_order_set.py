...
class Ui_order(QtGui.QDialog):
    def __init__(self, parent=None):
        ...
        QtCore.QObject.connect(
            self.button_up, QtCore.SIGNAL(_fromUtf8("clicked()")), self.up
        )
        QtCore.QObject.connect(
            self.button_down, QtCore.SIGNAL(_fromUtf8("clicked()")), self.down
        )
        QtCore.QObject.connect(
            self.buttonreset, QtCore.SIGNAL(_fromUtf8("clicked()")), self.reset
        )
        QtCore.QObject.connect(
            self.ui.button_configure,
            QtCore.SIGNAL(_fromUtf8("clicked()")),
            self.set_configure,
        )
        QtCore.QObject.connect(
            self.ui.button_configure_s,
            QtCore.SIGNAL(_fromUtf8("clicked()")),
            self.configure_strategy,
        )

        QtCore.QObject.connect(
            self.ui.button_save, QtCore.SIGNAL(_fromUtf8("clicked()")), self.set_save
        )
        QtCore.QObject.connect(
            self.ui.combobox_job,
    ...
    def configure_strategy(self):
        ...
        os.system(
            ...
            + os.path.join(
                com.root_path,
                "strategy/" + "strategy_" + paramgroup.Pgs_type[self.job] + ".py",
            )
        )
