...
class ImportFitFromFullProf:
    ...
    def ImportFile(self, fit):
        ...
        if self.ReadIndex != len(self.LineContent) -1:
            ...
            raise NotImplementedError("Line Number Incorrect at the End of File")
        else:
        ...
        for line in original:
            try:
                ...
                if line2 == "":
                    ...
                    # data line
                    templine1 = line.split('!')         # delete comments after !
                    templine2 = templine1[0].split('#') # delete comments after #
                ...
                print "Line "+str(linenumber)+":\t" + line + "\twith items = " + str(len(line1))
                #s = ""
                #lineindex = 1
                #for l in original[:]:
                #    s += str(lineindex) + ":   " + l
        ...
        if words[0] == "NPATT":
            ...
            if 0:  # 0/1 List just notice Fullprof to refine or exclude, but not affect File structure
                if fit.banklist != [] and NPATT != len(fit.banklist):
                    errmsg = "Line 2: NPATT = %5d Not Consistent With 1/0 List"%(NPATT)
                    raise RietPCRError(errmsg)
    ...
    def ReadBlock2(self, fit):
        ...
        if fit.get("Cry") == 0:
            for pattern in fit.get("Pattern"):
                ...
                if pattern.get("Uni") == 0: 
                    # 2theta
                    if len(words) not in [9,10]: # The 2nd-muR is added after 2011 Fullprof, 
                                                # so it may be 9 or 10 numbers
                        warning.PCRFormatItemError("2", "8-"+str(pattern), self.LineContent[index])
                        return False
        ...
        if self.Style == "new":
            ...
            for pattern in fit.get("Pattern"):
                ...
                if pattern.get("Uni") == 0: 
                    if len(words) != 5:
                        warning.PCRFormatItemError("2", "9-"+str(pattern), self.LineContent[index])
                        return False
            ...
            if pattern.get("Uni") == 0:
                ...
                if len(words) != 11:
                    warning.PCRFormatItemError("2", "9 old,  Uni =" + str(pattern.get("Uni")), self.LineContent[index])
                    return False
            ...
            for pattern in fit.get("Pattern"):
                ...
                if pattern.get("Nba") >= 2 or pattern.get("Nba") < -4:
                    ...
                    if pattern.get("Nba") == 2:
                        background = PatternModule.BackgroundUserDefinedLinear(None)
                    else:
                        background = PatternModule.BackgroundUserDefinedCubic(None)
                    ...
                    for l in xrange(0, linetoread):
                        ...
                        try:
                            ...
                            self.setRefine(fit, background, "BCK", bakval, refinecode, l)
                        except KeyError, err:
            ...
            if pattern.get("Nsc") != 0:
                ...
                for sc in xrange(1, lineno+1):
                    ...
                    if pattern.get("Job") == 0 or pattern.get("Job") == 2: 
                        scatterfactor = PatternModule.ScatterFactorXray(pattern)
                    #elif fit.Patterns[pat].Job == 1 or fit.Patterns[pat].Job == 3:
                    elif pattern.Job == 1 or pattern.Job == 3:
                        scatterfactor = PatternModule.ScatterFactorNeutron(pattern)
                    else:
                    ...
                    if scatterfactor.__class__.__name__ == "ScatterFactorXray": 
                        ...
                        if pattern.get("Nsc") > 0 and scatterfactor.get("ITY") == 0:
                            ...
                            words = self.SplitNewLine(index)
                            if len(words) != 9:
                                warning.PCRFormatItemError("2", "12b", "xray "+str(words))
                            # init
                            formfactor = PatternModule.FormFactorXray(scatterfactor)
                            ...
                            # init
                            formfactor = PatternModule.FormFactorTable(scatterfactor)
                            scatterfactor.set("FormFactor", formfactor)
                            # read
                        ...
                        if pattern.get("Nsc") > 0 and scatterfactor.get("ITY") == 1:
                            ...
                            words = self.SplitNewLine(index)
                            if len(words) != 7:
                                warning.PCRFormatItemError("2", "12b", "neutron "+str(words))
                            # init
                            formfactor = PatternModule.FormFactorNeutron(scatterfactor)
                            scatterfactor.set("FormFactor", formfactor)
                            # read
        ...
        for pat, pattern in enumerate(fit.get("Pattern")):
            if fit.get("Cry") == 0 and pattern.get("Uni") == 0:    # 2theta
                ...
                words = self.SplitNewLine(index)
                if len(words) != 9:
                    warning.PCRFormatItemError("2", "2theta: Line 14 Lenth = 9", str(words))
                #print("Zero"+str(words[0])+str(words[1]))
                self.setRefine(fit, pattern, "Zero", toFloat(words[0]), toFloat(words[1]))
                self.setRefine(fit, pattern, "Sycos",  toFloat(words[2]), toFloat(words[3]))
                self.setRefine(fit, pattern, "Sysin", toFloat(words[4]), toFloat(words[5]))
                self.setRefine(fit, pattern, "Lambda", toFloat(words[6]), toFloat(words[7]))
                More = int(words[8])
                if More != 0:
                ...
                if More != 0:
                    ...
                    if pattern.get("LPFactor").get("Ilo") != 0:
                        raise RietError("readpcrfile @ Line 15:  not allowed Ilo = " + str(pattern.get("Ilo")))
                    ...
                    words = self.SplitNewLine(index)
                    if len(words) != 6:
                        warning.PCRFormatItemError("2", "2theta: 15", str(words))
                    self.setRefine(fit, micabs, "P0",  toFloat(words[0]), toFloat(words[1]))
                    self.setRefine(fit, micabs, "CP",  toFloat(words[2]), toFloat(words[3]))
                    self.setRefine(fit, micabs, "TAU", toFloat(words[4]), toFloat(words[5]))
                ...
                if len(words) != 7:
                    ...
                    warning.PCRFormatItemError("2", "T.O.F: 14 pat" + str(pattern), str(words))
                self.setRefine(fit, pattern, "Zero", toFloat(words[0]), toFloat(words[1]))
                self.setRefine(fit, pattern, "Dtt1", toFloat(words[2]), toFloat(words[3]))
                self.setRefine(fit, pattern, "Dtt2", toFloat(words[4]), toFloat(words[5]))
                ...
                if pattern.get("Npr") == 10:
                    ...
                    words = self.SplitNewLine(index)
                    if len(words) != 5:
                        warning.PCRFormatItemError("2", "T.O.F: 16", self.LineContent[index])
                    ...
                    words = self.SplitNewLine(index)
                    if len(words) != 5:
                        warning.PCRFormatItemError("2", "T.O.F: 16-code", self.LineContent[index])
                    self.setRefine(fit, pattern, "Zerot",  zerot, toFloat(words[0]))
                    self.setRefine(fit, pattern, "Dtt1t",  dtt1t, toFloat(words[1]))
                    self.setRefine(fit, pattern, "Dtt2t",  dtt2t, toFloat(words[2]))
                    self.setRefine(fit, pattern, "xcross", xcross, toFloat(words[3]))
                    self.setRefine(fit, pattern, "Width",  width, toFloat(words[4]))
                ...
                words = self.SplitNewLine(index)
                if len(words) != 7:
                    warning.PCRFormatItemError("2", "Energy Dispersive: 14", str(words))
                codeindex = self.CodeWord(toFloat(words[0]), toFloat(words[1]), fit)
                #fit.Patterns[pat].powderData.Zero  = (toFloat(words[0]), codeindex)
                pattern.powderData.Zero  = (toFloat(words[0]), codeindex)
                codeindex = self.CodeWord(toFloat(words[2]), toFloat(words[3]), fit)
                #fit.Patterns[pat].powderData.StE1 = (toFloat(words[2]), codeindex)
                pattern.powderData.StE1 = (toFloat(words[2]), codeindex)
                codeindex = self.CodeWord(toFloat(words[4]), toFloat(words[5]), fit)
    ...
    def ReadBlock3(self, fit):
        ...
        for pha, phase in enumerate(fit.get("Phase")):
            ...
            if len(words) < 1 and len(words) > 3:
                warning.PCRFormatItemError("3", "18 phase "+str(pha), str(words))
            # phase.PHSNM = words[0]
            ...
            if self.Style == "new":
                ...
                if More != 0:
                    ...
                    if len(words) != 6 and len(words) != 7 :
                        ...
                        errmsg += "Line %-10s: Jvi, Jdi, Hel, Sol, Mom, Ter, N_Domains"% ("19-1")
                        print errmsg
                        warning.PCRFormatItemError("3", "19-1", self.LineContent[index])
                ...
                for pattern in fit.get("Pattern"):
                    ...
                    if usecontribution:
                        if pattern.get("Uni") == 0:
                            contribution = ContributionModule.Contribution2Theta(fit, pattern, phase)
                        else:
                            contribution = ContributionModule.ContributionTOF(fit, pattern, phase)
                ...
                for pattern in fit.get("Pattern"):
                    ...
                    if contribution is not None:
                        ...
                        if len(words) < 3 and len(words) > 5:
                            warning.PCRFormatItemError("3", "19-3: new", str(words))
                        try:
                            ...
                            errmsg  = str(err)+"\n"
                            errmsg += "pcrFileRead:  pcr File Error in Line 19-3  %-40s"%(self.LineContent[index])
                            errmsg += "\nthis error occurs in Contribution Section on Pattern %-5s"%(str(patternindex))
                            errmsg += "\nPrevious Line : %-40s" %(self.LineContent[index-1])
                            errmsg += "\nThis line     : %-40s" %(self.LineContent[index])
                            errmsg += "\nNext line     : %-40s" %(self.LineContent[index+1])
                            errmsg += "\nCorrect Format: %-40s" %(LineDescription["19-3"])
                        ...
                        words = self.SplitNewLine(index)
                        if len(words) != 7:
                            warning.PCRFormatItemError("3", "19-4: new", str(words))
                        ...
                        if contribution.get("Irf") == 4:
                            ...
                            raise NotImplementedError("In multiple pattern, Irf == 4 raise an exception! Manual states")
                ...
                if len(words) != 15:
                    print str(words)
                    warning.PCRFormatItemError("3", "19", self.LineContent[index]+"  Next Line: "+self.LineContent[index+1])
                phase.set("Nat", int(words[0]))
                phase.set("Dis", int(words[1]))
                phase.set("MomMA", int(words[2]))
                pr1 = toFloat(words[3]) 
                ...
                if More != 0:
                    ...
                    if len(words) < 11 or len(words) > 13:
                        ...
                        print errmsg
                        warning.PCRFormatItemError("3", "19-1: old", self.LineContent[index])
                    phase.set("Jvi", int(words[0]))
                    phase.set("Jdi", int(words[1]))
                    hel = int(words[2])
                    if hel == 0:
            ...
            if (phase.get("Jbt") == 10 or phase.get("Jbt") == -10) and IsMagnetic:
                ...
                words = self.SplitNewLine(index)
                if len(words) != NS+1:
                    warning.PCRFormatItemError("3", "23", str(words) + " Wrong Length = " + str(len(words)))
            ...
            if phase.get("Isy") != 0 and phase.get("Jbt") != 15:
                ...
                words = self.SplitNewLine(index)
                if len(words) != 4:
                    warning.PCRFormatItemError("3", "23 choice 1", str(words) + " Wrong Length = " + str(len(words)))
            ...
            if phase.get("Isy") != 0 and phase.get("Jbt") == 15:
                ...
                words = self.SplitNewLine(index)
                if len(words) != 5:
                    warning.PCRFormatItemError("3", "23 choice 2", str(words) + " Wrong Length = " + str(len(words)))
                # symmetry operator init
                symmopschool = PhaseModule.OperatorSetSymmetry(None)
            ...
            if phase.get("Isy") == -2  and (phase.get("Jbt") == 1 or phase.get("Jbt") == -1):
                ...
                words = self.SplitNewLine(index)
                if len(words) != 5:
                    warning.PCRFormatItemError("3", "23 choice 3", str(words) + " Wrong Length = " + str(len(words)))
                # symmetry operator init
            ...
            if readline23 > 1:
                raise NotImplementedError("Line 23:  Read Line 23 More than Once")
            ...
            if phase.get("Isy") == -2:
                ...
                if len(words) <= 0 or len(words) > 9:
                    warning.PCRFormatItemError("3", "23-1", str(words) + " Wrong Length = " + str(len(words)))
                # init
                symmopschool = phase.get("OperatorSetSymmetry")
                icomplobj = PhaseModule.Icompl(symmopschool, symmopschool.get("N_Bas"))
            ...
            if phase.get("Isy") == 1 or  phase.get("Isy") == -1:
                ...
                raise NotImplementedError("Line 24: No idea how to apply Isy = " + str(phase.get("Isy")))
            ...
            if phase.get("Jbt") == 0:
                ...
                for at in xrange(0, phase.get("Nat")):
                    ...
                    if atom.get("N_t") == 0:
                        ...
                        # init object
                        atomicdisplace = AtomModule.AtomicDisplacementFactorIsotropic(None)
                        atom.set("AtomicDisplacementFactor", atomicdisplace)
                        # set value
                        self.setRefine(fit, atomicdisplace, "Biso", nBiso, biso_code)
                        ...
                        # init object
                        atomicdisplace = AtomModule.AtomicDisplacementFactorAnisotropic(None)
                        atom.set("AtomicDisplacementFactor", atomicdisplace)
                        # set value
                        self.setRefine(fit, atomicdisplace, "Biso", nBiso, biso_code)
                        # Line 25-3-1
                        ...
                        for i in [(1,1), (2,2), (3,3), (1,2), (1,3), (2,3)]:
                            ...
                            self.setRefine(fit, atomicdisplace, param_name, B[i], code)
                            count = count + 1

                    elif atom.get("N_t") == 4:
                        # init object
                        atomicdisplace = AtomModule.AtomicDisplacementFactorFormfactor(None)
                        atom.set("AtomicDisplacementFactor", atomicdisplace)
                        # Line 25-3
                        f     = {}
                        words = {}
                        ...
                        for i in xrange(1, 7+1):
                            code           = toFloat(words[count])
                            self.setRefine(fit, atomicdisplace, param_name, f[i], code)
                            count = count + 1
                        # Line 25-7.8
                        if atom.get("Typ") == "SASH":
                            ...
                            if len(words)!= 2*atom.Ncoeff and len(words)!=3*atom.Ncoeff:
                                warning.PCRFormatItemError("3", "25-8 Jbt=0", str(words))
                ...
                if refinebiso is True and self.Style == "old":
                    for atom in phase.get("Atom"):
                        ...
                        if biso:
                            ...
                            errmsg += "Biso's class = %-30s"% (biso.__class__.__name__)
                            raise RietError(errmsg)
                ...
                for at in xrange(0, phase.get("Nat")):
                    ...
                    words = self.SplitNewLine(index)
                    if len(words) != 12:
                        warning.PCRFormatItemError("3", "25-1 Jbt=1", str(words))
                    atom.set("Name", words[0])
                    atom.set("Typ",  words[1])
                    ...
                    words = self.SplitNewLine(index)
                    if len(words) != 8:
                        warning.PCRFormatItemError("3", "25-2 Jbt=1", str(words))
                    code      = toFloat(words[0])
                    ...
                    if len(words) != 7:
                        warning.PCRFormatItemError("3", "25-3 Jbt=1", str(words))
                    IX = toFloat(words[0])
                    IY = toFloat(words[1])
                    IZ = toFloat(words[2])
                    B11 = toFloat(words[3])
                    ...
                    words = self.SplitNewLine(index)
                    if len(words) != 7:
                        warning.PCRFormatItemError("3", "25-4 Jbt=1", str(words))
                    code      = toFloat(words[0])
                ...
                for at in xrange(0, phase.get("Nat")):
                    ...
                    words = self.SplitNewLine(index)
                    if len(words) != 10:
                        warning.PCRFormatItemError("3", "25-1 Jbt=-4", str(words))
                    atom.Atom = words[0]
                    atom.Typ  = words[1]
                    nP = {}
                    ...
                    words = self.SplitNewLine(index)
                    if len(words) != 8:
                        warning.PCRFormatItemError("3", "25-2 Jbt=-4", str(words))
                    for p in xrange(1, 8+1):
                    ...
                    # Line 25-3
                    if len(words) != 7:
                        warning.PCRFormatItemError("3", "25-3 Jbt=-4", str(words))
                    nP = {}
                    ...
                    words = self.SplitNewLine(index)
                    if len(words) != 7:
                        warning.PCRFormatItemError("3", "25-4 Jbt=-4", str(words))
                ...
                for at in xrange(0, phase.get("Nat")):
                    ...
                    if (atom.get("N_t") == 1 or atom.get("N_t") == 3) and phase.get("Isy") != -2:
                        ...
                        if len(words)!= 7:
                            print "bugged line:\t" + str(words)
                            warning.PCRFormatItemError4("3", "25b-3", str(words), 7, "MagPar")
                        RX = toFloat(words[0])
                        RY = toFloat(words[1])
                        RZ = toFloat(words[2])
                        IX = toFloat(words[3])
                        ...
                        if len(words)!= 7:
                            ...
                            warning.PCRFormatItemError("3", "25b-4", str(words))
                        code      = toFloat(words[0])
                        self.setRefine(fit, magmoment, "RX", RX, code)
                        code      = toFloat(words[1])
                        self.setRefine(fit, magmoment, "RY", RY, code)
                        ...
                        words = self.SplitNewLine(index)
                        if len(words)!= 7:
                            warning.PCRFormatItemError4("3", "25b-3", (words), 7, "MagPar")
                        RM = toFloat(words[0])
                    ...
                    if (atom.get("N_t") == 2 or atom.get("N_t") == 3 or atom.get("N_t") == -3) and phase.get("Isy") == -2:
                        ...
                        words = self.SplitNewLine(index)
                        if len(words)!= 6:
                            warning.PCRFormatItemError("3", "25b-5", str(words))
                        counter = 0
                        ...
                        words = self.SplitNewLine(index)
                        if len(words)!= 6:
                            warning.PCRFormatItemError("3", "25b-6", str(words))
                        counter = 0
                        for b in [(1,1), (2,2), (3,3), (1,2), (1,3), (2,3)]:
                            code      = toFloat(words[counter])
                            param_name = "B"+str(b[0])+str(b[1])
                            self.setRefine(fit, atomicdisplace, param_name, B[b], code)
                    # 25b-7~8
                    if atom.get("N_t") == 4:
                        # init
                        atomicdisplace = AtomModule.AtomicDisplacementFactorFormfactor(None)
                        ...
                        words = self.SplitNewLine(index)
                        if len(words)!= 7:
                            warning.PCRFormatItemError("3", "25b-7", str(words))
                        for i in xrange(1, 7+1):
                        ...
                        words = self.SplitNewLine(index)
                        if len(words)!= 7:
                            warning.PCRFormatItemError("3", "25b-8", str(words))
                        for i in xrange(1, 7+1):
                        ...
                        words = self.SplitNewLine(index)
                        if len(words)!= 7:
                            warning.PCRFormatItemError("3", "25b-9", str(words))
                        ...
                        words = self.SplitNewLine(index)
                        if len(words)!= 7:
                            warning.PCRFormatItemError("3", "25b-10", str(words))
                        for i in xrange(8, 14+1):
                ...
                for at in xrange(0, phase.get("Nat")):
                    ...
                    if (atom.get("N_t") == 1 or atom.get("N_t") == 3):
                        ...
                        words = self.SplitNewLine(index)
                        if len(words)!= 7:
                            warning.PCRFormatItemError4("3", "25b-4", str(words))
                        code      = toFloat(words[0])
                        ...
                        for dvk in xrange(1, abs(Ndvk)+1):
                            ...
                            words = self.SplitNewLine(index)
                            if len(words)!= 9:
                                warning.PCRFormatItemError4("3", "25b-3: Jbt=+/-15 Ndvk!=0 "+ "Line " + str(dvk), (words), 7, "MagPar")
                ...
                for pattern in fit.get("Pattern"):
                    ...
                    if pattern.get("Uni") == 0:
                        if contribution.get("Npr") != 11:
                            profile = ContributionModule.Profile2ThetaNonSPV(None)
                        else:
                    ...
                    if pattern.get("Uni") == 0:    # 2theta
                        # Line 26
                        if len(words) != 7:
                            warning.PCRFormatItemError("3", "26 2theta", str(words))
                        try:
                            ...
                            errmsg += str(err)
                            raise RietPCRError(errmsg)
                        strainparameter.set("StrainModelSelector", int(words[6]))
                        # Line 26-1
                        index = index+1
                        words = self.SplitNewLine(index)
                        if len(words) != 6:
                            warning.PCRFormatItemError("3", "26-1 2theta", str(words))
                        code      = toFloat(words[0])
                        ...
                        code      = toFloat(words[3])
                        self.setRefine(fit, strainparameter, "Str1", Str1, code)
                        code      = toFloat(words[4])
                        self.setRefine(fit, strainparameter, "Str2", Str2, code)
                        code      = toFloat(words[5])
                        self.setRefine(fit, strainparameter, "Str3", Str3, code)
                        ...
                        if contribution.get("Npr") != 11:
                            ...
                            words = self.SplitNewLine(index)
                            if len(words) != 8:
                                warning.PCRFormatItemError("3", "27 2theta", str(words))
                            U = toFloat(words[0])
                            V = toFloat(words[1])
                            W = toFloat(words[2])
                            X = toFloat(words[3])
                            ...
                            words = self.SplitNewLine(index)
                            if len(words) != 7:
                                warning.PCRFormatItemError("3", "27-1 2theta", str(words))
                            code      = toFloat(words[0])
                            ...
                            code      = toFloat(words[5])
                            self.setRefine(fit, profile, "GausSiz", GausSiz, code)
                            code      = toFloat(words[6])
                            self.setRefine(fit, profile, "LorSiz", LorSiz, code)
                        else:
                            # Line 27 
                            index = index + 1
                            words = self.SplitNewLine(index)
                            if len(words) != 8:
                                warning.PCRFormatItemError("3", "27 2theta Npr = 11", str(words))                                           # read
                            UL  = toFloat(words[0])
                            VL  = toFloat(words[1])
                            WL  = toFloat(words[2])
                            XL  = toFloat(words[3])
                            ...
                            words = self.SplitNewLine(index)
                            if len(words) != 5:
                                warning.PCRFormatItemError("3", "27-3 2theta Npr = 11", str(words))                     
                            Ur  = toFloat(words[0])
                            Vr  = toFloat(words[1])
                            Wr  = toFloat(words[2])
                            ...
                            words = self.SplitNewLine(index)
                            if len(words) != 5:
                                warning.PCRFormatItemError("3", "27-4 2theta", str(words))
                            code      = toFloat(words[0])
                        ...
                        # Line 26 
                        if len(words) != 7:
                            warning.PCRFormatItemError("3", "26 TOF", str(words))
                        ...
                        self.setRefine(fit, contribution, "Scale", Scale, code)
                        code      = toFloat(words[1])
                        self.setRefine(fit, contribution, "Extinct", Extinc, code)
                        code      = toFloat(words[2])
                        self.setRefine(fit, contribution, "Bov", Bov, code)
                        code      = toFloat(words[3])
                        self.setRefine(fit, strainparameter, "Str1", Str1, code)
                        ...
                        words = self.SplitNewLine(index)
                        if len(words) != 8:
                            warning.PCRFormatItemError("3", "27 TOF", str(words))                     
                        ...
                        words = self.SplitNewLine(index)
                        if len(words) != 7:
                            warning.PCRFormatItemError("3", "27-1 TOF", str(words))
                        # Read
                        code      = toFloat(words[0])
                        ...
                        words = self.SplitNewLine(index)
                        if len(words) != 5:
                            warning.PCRFormatItemError("3", "27-2 TOF", str(words))                     
                    ...
                    if pattern.get("Uni") == 0:    # 2theta
                        ...
                        if len(words) != 6 and len(words) != 8:
                            warning.PCRFormatItemError("3", "30 2theta", str(words))                     
                        ...
                        words = self.SplitNewLine(index)
                        if len(words) != 6 and len(words) != 8:
                            warning.PCRFormatItemError("3", "30-1 2theta", str(words))
                        code      = toFloat(words[0])
                        ...
                        for pa in xrange(1, 4+1):
                            ...
                            self.setRefine(fit, asymparam, param_name, PA[pa], code)
                        # optional terms
                        if contribution.get("Npr") == 7:
                            # read
                            if len(words) >= 8: 
                        ...
                        if pattern.get("Ratio") < 0:
                            ...
                            words = self.SplitNewLine(index)
                            if len(words) != 3:
                                warning.PCRFormatItemError4("3", "32 2theta", (words), 3, "<-")                     
                            U2  = toFloat(words[0])
                            V2  = toFloat(words[1])
                            W2  = toFloat(words[2])
                            # Line 32-1
                            ...
                            words = self.SplitNewLine(index)
                            if len(words) != 3:
                                warning.PCRFormatItemError4("3", "30-1 2theta", (words), 3, "<-")
                            code      = toFloat(words[0])
                        ...
                        if contribution.get("Npr") == 4 or contribution.get("Npr") > 7:
                            ...
                            words = self.SplitNewLine(index)
                            if len(words) != 4:
                                warning.PCRFormatItemError("3", "33 2theta", str(words))
                        ...
                        words = self.SplitNewLine(index)
                        if len(words) != 6:
                            warning.PCRFormatItemError("3", "30 T.O.F.", str(words))                     
                        Pref1 = toFloat(words[0])
                        Pref2 = toFloat(words[1])
                        ...
                        words = self.SplitNewLine(index)
                        if len(words) != 6:
                            warning.PCRFormatItemError("3", "30-1 T.O.F.", str(words))
                        code      = toFloat(words[0])
                        self.setRefine(fit, preferorient, "Pref1", Pref1, code)
                        code      = toFloat(words[1])
                        self.setRefine(fit, preferorient, "Pref2", Pref2, code)
                        ...
                        if contribution.get("Npr") == 10:
                            ...
                            words = self.SplitNewLine(index)
                            if len(words) != 4:
                                warning.PCRFormatItemError("3", "31 T.O.F.", str(words))                     
                            ...
                            words = self.SplitNewLine(index)
                            if len(words) != 4:
                                warning.PCRFormatItemError("3", "31-1 T.O.F.", str(words))
                            code      = toFloat(words[0])
                            self.setRefine(fit, expdecay, "ALPH0T", ALPH0T, code)
                            code      = toFloat(words[1])
                            self.setRefine(fit, expdecay, "BETA0T", BETA0T, code)
                            code      = toFloat(words[2])
                            self.setRefine(fit, expdecay, "ALPH1T", ALPH1T, code)
                            code      = toFloat(words[3])
                            self.setRefine(fit, expdecay, "BETA1T", BETA1T, code)
                        # Line 33
                        index = index+1
                        words = self.SplitNewLine(index)
                        if len(words) != 4:
                            warning.PCRFormatItemError4("3", "33 T.O.F", (words), 4, "ABS")
                    ...
                    if phase.get("Sol") != 0:
                        ...
                        for shf in xrange(1, 3+1):
                            ...
                            self.setRefine(fit, shiftmodel, param_name, SHF[shf], code)
                        # Line 35
                        ModS = shiftmodel.get("ModS")
                        if ModS == 1 or ModS == -1:

                            # extend
                            shiftmodelangle = ContributionModule.ShiftParameterAngle(None)
                            ...
                            words = self.SplitNewLine(index)
                            if len(words) != 3:
                                warning.PCRFormatItemError("3", "35 (1)", str(words))
                            ...
                            # extend
                            shiftmodeluser = ContributionModule.ShiftParameterUserdefined(None)
                            ...
                            for m in xrange(-ModS):
                                ...
                                words = self.SplitNewLine(index)
                                if len(words) != 7:
                                    warning.PCRFormatItemError("3", "35 (2)", str(words))
                                for s in xrange(1, 5+1):
                                    param_name = "n"+str(s)
                                    shift.set(param_name, int(words[s-1]))
                                Shift     = toFloat(words[5])
                                code      = toFloat(words[6])
                                self.setRefine(fit, shift, "Shift", Shift, code)

                        elif ModS == 101:

                            # extend
                            shiftmodellaue = ContributionModule.ShiftParameterLaue(None, "-1")
                            shiftmodellaue.extend(shiftmodel)
                            shiftmodel = shiftmodellaue
                            # read
                            index = self.Line35ReadLaueClass(fit, shiftmodel, index, [6, 5, 5, 5])

                        elif ModS == 102:

                            # extend
                            shiftmodellaue = ContributionModule.ShiftParameterLaue(None, "1 2/m 1")
                            shiftmodellaue.extend(shiftmodel)
                            shiftmodel = shiftmodellaue
                            # read
                            index = self.Line35ReadLaueClass(fit, shiftmodel, index, [4, 5, 4])

                        elif ModS == -102:

                            # extend
                            shiftmodellaue = ContributionModule.ShiftParameterLaue(None, "1 1 2/m")
                            shiftmodellaue.extend(shiftmodel)
                            shiftmodel = shiftmodellaue
                            # read
                            index = self.Line35ReadLaueClass(fit, shiftmodel, index, [4, 5, 4])
                            ...
                            # extend
                            shiftmodellaue = ContributionModule.ShiftParameterLaue(None, "mmm")
                            shiftmodellaue.extend(shiftmodel)
                            shiftmodel = shiftmodellaue
                            # read
                            index = self.Line35ReadLaueClass(fit, shiftmodel, index, [3, 6])

                        elif ModS == 104:

                            # extend
                            shiftmodellaue = ContributionModule.ShiftParameterLaue(None, "4/m")
                            shiftmodellaue.extend(shiftmodel)
                            shiftmodel = shiftmodellaue
                            # read
                            index = self.Line35ReadLaueClass(fit, shiftmodel, index, [2 ,4])

                        elif ModS == 105:

                            # extend
                            shiftmodellaue = ContributionModule.ShiftParameterLaue(None, "4/mmm")
                            shiftmodellaue.extend(shiftmodel)
                            shiftmodel = shiftmodellaue
                            # read
                            index = self.Line35ReadLaueClass(fit, shiftmodel, index, [2 ,4])
                            ...
                            # extend
                            shiftmodellaue = ContributionModule.ShiftParameterLaue(None, "-3 R")
                            shiftmodellaue.extend(shiftmodel)
                            shiftmodel = shiftmodellaue
                            # read
                            index = self.Line35ReadLaueClass(fit, shiftmodel, index, [2 ,4])
                            ...
                            # extend
                            shiftmodellaue = ContributionModule.ShiftParameterLaue(None, "-3m R")
                            shiftmodellaue.extend(shiftmodel)
                            shiftmodel = shiftmodellaue
                            # read
                            index = self.Line35ReadLaueClass(fit, shiftmodel, index, [2 ,4])
                            ...
                            # extend
                            shiftmodellaue = ContributionModule.ShiftParameterLaue(None, "-3 H")
                            shiftmodellaue.extend(shiftmodel)
                            shiftmodel = shiftmodellaue
                            # read
                            index = self.Line35ReadLaueClass(fit, shiftmodel, index, [2 ,3])

                        elif ModS == 109:

                            # extend
                            shiftmodellaue = ContributionModule.ShiftParameterLaue(None, "-3m1")
                            shiftmodellaue.extend(shiftmodel)
                            shiftmodel = shiftmodellaue
                            # read
                            index = self.Line35ReadLaueClass(fit, shiftmodel, index, [2 ,3])
                            ...
                            # extend
                            shiftmodellaue = ContributionModule.ShiftParameterLaue(None, "-31m")
                            shiftmodellaue.extend(shiftmodel)
                            shiftmodel = shiftmodellaue
                            # read
                            index = self.Line35ReadLaueClass(fit, shiftmodel, index, [2 ,3])
                            ...
                            # extend
                            shiftmodellaue = ContributionModule.ShiftParameterLaue(None, "6/m")
                            shiftmodellaue.extend(shiftmodel)
                            shiftmodel = shiftmodellaue
                            # read
                            index = self.Line35ReadLaueClass(fit, shiftmodel, index, [2 ,3])
                            ...
                            # extend
                            shiftmodellaue = ContributionModule.ShiftParameterLaue(None, "6/mmm")
                            shiftmodellaue.extend(shiftmodel)
                            shiftmodel = shiftmodellaue
                            # read
                            index = self.Line35ReadLaueClass(fit, shiftmodel, index, [2 ,3])
                            ...
                            # extend
                            shiftmodellaue = ContributionModule.ShiftParameterLaue(None, "m3")
                            shiftmodellaue.extend(shiftmodel)
                            shiftmodel = shiftmodellaue
                            # read
                            index = self.Line35ReadLaueClass(fit, shiftmodel, index, [1 ,2])

                        elif ModS == 114:

                            # extend
                            shiftmodellaue = ContributionModule.ShiftParameterLaue(None, "m3m")
                            shiftmodellaue.extend(shiftmodel)
                            shiftmodel = shiftmodellaue
                            # read
                            index = self.Line35ReadLaueClass(fit, shiftmodel, index, [1 ,2])
                    ...
                    if SizeModel == 1 or SizeModel == -1:
                        ...
                        # init 
                        sizemodel = ContributionModule.SizeModelCylindrical(None)
                        contribution.set("SizeModel", sizemodel)
                        # read
                        index = index+1
                        words = self.SplitNewLine(index)
                        if len(words) != 3:
                            warning.PCRFormatItemError("3", "36 Size-Model = +/-1", str(words))
                        for shf in xrange(1, 3+1):
                        ...
                        words = self.SplitNewLine(index)
                        if len(words) != 6:
                            warning.PCRFormatItemError("6", "36 Size-Model < -1", str(words))
                        ...
                        sizemodel =  ContributionModule.SizeModelSpherical(None, "2/m")
                        contribution.set("SizeModel", sizemodel)
                        shlist = [ ["Y00" , "Y22+", "Y22-", "Y20", "Y44+", "Y44-"], ["Y42+", "Y42-", "Y40"] ]
                        index  = self.Line36ReadSphericalSizeModel(fit, sizemodel, index, shlist)
                        # index = self.Line36ReadLaueClass(fit, sizemodel, index, [6, 3])

                    elif SizeModel == 16:

                        sizemodel =  ContributionModule.SizeModelSpherical(None, "-3 m H")
                        contribution.set("SizeModel", sizemodel)
                        index = self.Line36ReadLaueClass(fit, sizemodel, index, [6, 1])

                    elif SizeModel == 17:

                        sizemodel =  ContributionModule.SizeModelSpherical(None, "m3")
                        contribution.set("SizeModel", sizemodel)
                        index = self.Line36ReadLaueClass(fit, sizemodel, index, [5])

                    elif SizeModel == 18:

                        sizemodel =  ContributionModule.SizeModelSpherical(None, "mmm")
                        contribution.set("SizeModel", sizemodel)
                        index = self.Line36ReadLaueClass(fit, sizemodel, index, [6])

                    elif SizeModel == 19:

                        sizemodel =  ContributionModule.SizeModelSpherical(None, "6/m")
                        contribution.set("SizeModel", sizemodel)
                        index = self.Line36ReadLaueClass(fit, sizemodel, index, [6])

                    elif SizeModel == 20:

                        sizemodel =  ContributionModule.SizeModelSpherical(None, "-3")
                        contribution.set("SizeModel", sizemodel)
                        index = self.Line36ReadLaueClass(fit, sizemodel, index, [5])

                    elif SizeModel == 21:

                        sizemodel =  ContributionModule.SizeModelSpherical(None, "4/m")
                        contribution.set("SizeModel", sizemodel)
                        index = self.Line36ReadLaueClass(fit, sizemodel, index, [6, 2])

                    elif SizeModel == 22:

                        sizemodel =  ContributionModule.SizeModelSpherical(None, "-1")
                        contribution.set("SizeModel", sizemodel)
                        index = self.Line36ReadLaueClass(fit, sizemodel, index, [6])

                    else:
                        if SizeModel != 0:
                            raise NotImplementedError("ModelSize Not Defined " + str(SizeModel))

                    # end --- if SizeModel != 0 Line 36

                    # Line 37 
                    meet = 0
                    StrainModelType = strainparameter.get("StrainModelSelector")
                    Str             = phase.get("Str")
                    ...
                    if StrainModelType == 7:
                        ...
                        words = self.SplitNewLine(index)
                        if len(words) != 3:
                            warning.PCRFormatItemError("3", "37 StrainModel == 7", str(words))
                        for shf in xrange(1, 3+1):
                    ...
                    if StrainModelType > 8 and Str == 0:
                        ...
                        words = self.SplitNewLine(index)
                        if len(words) != 5:
                            warning.PCRFormatItemError("5", "37 StrainModel > 8 & Str = 0", str(words))
                        ...
                        words = self.SplitNewLine(index)
                        if len(words) != 5:
                            warning.PCRFormatItemError("5", "37-1 StrainModel > 8 & Str = 0", str(words))
                        for shf in xrange(4, 8+1):
                            code       = toFloat(words[shf-4])
                            param_name = "Str"+str(shf)
                            self.setRefine(fit, strainmodel, param_name, STR[shf], code)
                    ...
                    if Str == -1 or Str == 2 or Str == 3:
                        ...
                        words = self.SplitNewLine(index)
                        if len(words) != 6:
                            warning.PCRFormatItemError("5", "37 Str = -1, 2, 3", str(words))
                        ...
                        words = self.SplitNewLine(index)
                        if len(words) != 6:
                            warning.PCRFormatItemError("5", "37-1 Str = -1, 2, 3", str(words))
                        for shf in xrange(1, 6+1):
                            code       = toFloat(words[shf-1])
                            param_name = "SZ"+str(shf)
                            self.setRefine(fit, sizemodel, param_name, SZ[shf], code)

                    if abs(Str) == 1 and StrainModelType == 1: 

                        meet = meet+1
                        # init
                        strainmodel = ContributionModule.StrainModelAnisotropic(None, "-1")
                        strainparameter.set("StrainModel", strainmodel)
                        # read
                        index = self.Line37ReadLaueClass(fit, strainmodel, index, [5, 5 ,5])

                    if abs(Str) == 1 and StrainModelType == 2: 

                        meet = meet+1
                        # init
                        strainmodel = ContributionModule.StrainModelAnisotropic(None, "1 2/m 1")
                        strainparameter.set("StrainModel", strainmodel)
                        # read
                        index = self.Line37ReadLaueClass(fit, strainmodel, index, [5, 4])

                    if abs(Str) == 1 and StrainModelType == -2: 

                        meet = meet+1
                        # init
                        strainmodel = ContributionModule.StrainModelAnisotropic(None, "1 1 2/m")
                        strainparameter.set("StrainModel", strainmodel)
                        # read
                        index = self.Line37ReadLaueClass(fit, strainmodel, index, [5, 4])
                    ...
                    if abs(Str) == 1 and StrainModelType == 3: 
                        ...
                        meet = meet+1
                        # init
                        strainmodel = ContributionModule.StrainModelAnisotropic(None, "mmm")
                        strainparameter.set("StrainModel", strainmodel)
                        # read
                        index = self.Line37ReadLaueClass(fit, strainmodel, index, [6])

                    if abs(Str) == 1 and StrainModelType == 4: 

                        meet = meet+1
                        # init
                        strainmodel = ContributionModule.StrainModelAnisotropic(None, "4/m")
                        strainparameter.set("StrainModel", strainmodel)
                        # read
                        index = self.Line37ReadLaueClass(fit, strainmodel, index, [4])

                    if abs(Str) == 1 and StrainModelType == 5: 

                        meet = meet+1
                        # init
                        strainmodel = ContributionModule.StrainModelAnisotropic(None, "4/mmm")
                        strainparameter.set("StrainModel", strainmodel)
                        # read
                        index = self.Line37ReadLaueClass(fit, strainmodel, index, [4])
                    ...
                    if abs(Str) == 1 and StrainModelType == 6: 
                        ...
                        meet = meet+1
                        # init
                        strainmodel = ContributionModule.StrainModelAnisotropic(None, "-3 R")
                        strainparameter.set("StrainModel", strainmodel)
                        # read
                        index = self.Line37ReadLaueClass(fit, strainmodel, index, [4])
                    ...
                    if abs(Str) == 1 and StrainModelType == 7: 
                        ...
                        meet = meet+1
                        # init
                        strainmodel = ContributionModule.StrainModelAnisotropic(None, "-3m R")
                        strainparameter.set("StrainModel", strainmodel)
                        # read
                        index = self.Line37ReadLaueClass(fit, strainmodel, index, [4])
                    ...
                    if abs(Str) == 1 and StrainModelType == 8: 
                        ...
                        meet = meet+1
                        # init
                        strainmodel = ContributionModule.StrainModelAnisotropic(None, "-3")
                        strainparameter.set("StrainModel", strainmodel)
                        # read
                        index = self.Line37ReadLaueClass(fit, strainmodel, index, [3])

                    if abs(Str) == 1 and StrainModelType == 9: 

                        meet = meet+1
                        # init
                        strainmodel = ContributionModule.StrainModelAnisotropic(None, "-3m1")
                        strainparameter.set("StrainModel", strainmodel)
                        # read
                        index = self.Line37ReadLaueClass(fit, strainmodel, index, [3])
                    ...
                    if abs(Str) == 1 and StrainModelType == 10: 
                        ...
                        meet = meet+1
                        # init
                        strainmodel = ContributionModule.StrainModelAnisotropic(None, "-31m")
                        strainparameter.set("StrainModel", strainmodel)
                        # read
                        index = self.Line37ReadLaueClass(fit, strainmodel, index, [3])
                    ...
                    if abs(Str) == 1 and StrainModelType == 11: 
                        ...
                        meet = meet+1
                        # init
                        strainmodel = ContributionModule.StrainModelAnisotropic(None, "6/m")
                        index = self.Line37ReadLaueClass(fit, strainmodel, index, [3])
                        strainparameter.set("StrainModel", strainmodel)
                        # read
                    ...
                    if abs(Str) == 1 and StrainModelType == 12: 
                        ...
                        meet = meet+1
                        # init
                        strainmodel = ContributionModule.StrainModelAnisotropic(None, "6/mmm")
                        strainparameter.set("StrainModel", strainmodel)
                        # read
                        index = self.Line37ReadLaueClass(fit, strainmodel, index, [3])
                    ...
                    if abs(Str) == 1 and StrainModelType == 13: 
                        ...
                        meet = meet+1
                        # init
                        strainmodel = ContributionModule.StrainModelAnisotropic(None, "m3")
                        strainparameter.set("StrainModel", strainmodel)
                        # read
                        index = self.Line37ReadLaueClass(fit, strainmodel, index, [2])

                    if abs(Str) == 1 and StrainModelType == 14: 

                        meet = meet+1
                        # init
                        strainmodel = ContributionModule.StrainModelAnisotropic(None, "m3m")
                        strainparameter.set("StrainModel", strainmodel)
                        # read
                        index = self.Line37ReadLaueClass(fit, strainmodel, index, [2])
                    ...
                    if (Str == 1 and StrainModelType == 0) or Str == 3:
                        ...
                        words = self.SplitNewLine(index)
                        if len(words) != 6:
                            warning.PCRFormatItemError("6", "37 Str=1 and Strain-model = 0, or Str = 3", str(words))
                        ...
                        words = self.SplitNewLine(index)
                        if len(words) != 6:
                            warning.PCRFormatItemError("6", "37-1 Str=1 and Strain-model = 0, or Str = 3", str(words))
                        for shf in xrange(4, 9+1):
                            code       = toFloat(words[shf-4])
                            param_name = "STR"+str(shf)
                            self.setRefine(fit, strainmodel, param_name, STR[shf], code)
                        ...
                        words = self.SplitNewLine(index)
                        if len(words) != 6:
                            warning.PCRFormatItemError("6", "37-2 Str=1 and Strain-model = 0, or Str = 3", str(words))
                        ...
                        words = self.SplitNewLine(index)
                        if len(words) != 6:
                            warning.PCRFormatItemError("3", "37-3 Str=1 and Strain-model = 0, or Str = 3", str(words))
                        for shf in xrange(10, 15+1):
                    ...
                    if meet > 1:
                        raise NotImplementedError("Line 37:  confusion... meet = " + str(meet))
                    ...
                    for line in xrange(1, contribution.get("Nsp_Ref")+1):
                        ...
                        words = self.SplitNewLine(index)
                        if len(words) != 10:
                            warning.PCRFormatItemError("3", "Line h k l nvk D-HG^2 Cod_D-HG^2 D-HL Cod_D-HL Shift Cod_Shif", str(words))
            ...
            for ion in self.ionList[:]:
                ...
                for atom in phase.get("Atom"):
                    ...
                    if symbol == atomsymbol:
                        ...
                        atom.set("Symbol",    symbol)
                        count += 1
                        messagespecial = "Special:  Atom Symbol = %-10s  Ion = %-10s"%(atomsymbol, symbol)
    ...
    def ReadBlock4(self, fit):
        ...
        for line in xrange(0, fit.get("Nre")):
            ...
            if Cry != 3 and (len(words) != 3 and len(words) != 6):
                warning.PCRFormatItemError("4", "Line 47 Cry != 3 - " + str(line), str(words))
            ...
            if variable is None:
                raise RietError("Block 4: Key " + str(NUMPAR) + "  Doesn't Exit in RefineList")
    ...
    def ReadBlock6(self, fit):
        ...
        for pattern in fit.get("pattern"):
            if pattern.get("Ppl") == 1:
                ...
                words = self.SplitNewLine(index)
                if len(words) != 2:
                    raise NotImplementedError("6", "50-" + str(pat), str(words))
    ...
    def SplitNewLine(self, index):
        try:
            ...
            for line in xrange(0, 200):
                if self.LineContent.has_key(line):
    ...
    def Line17Read(self, fit, pattern, index):
        ...
        if pattern.get("Nba") == -3 or pattern.get("Nba") == -4:      # 12-2theta  CHECK NEED TO ASK A QUESTION
            ...
            for i in xrange(0, 12):
                param_name = "BACK"
                self.setRefine(fit, background, param_name, tempvalues[i], tempcodes[i], i)
            ...
            if ( pattern.get ( "Nba" ) == -4 ):
                ...
                words = self.SplitNewLine(index)
                if len(words) != 6:
                    warning.PCRFormatItemError("2", "17-5: 12 coeff", str(words))
                for i in xrange(0, 6):
                    tempvalues[i] = toFloat(words[i])
                # 17-6
                index = index + 1
            ...
            for i in xrange(0, 6):
                ...
                self.setRefine(fit, background, param_name, tempvalues[i], tempcodes[i])
            # 17-3
            index = index + 1
            words = self.SplitNewLine(index)
            if len(words) != 6:
    ...
    def Line24ReadAlpha(self, index, operatorcombo, MagMat, DepMat):
        ...
        words = self.SplitNewLine(index)
        if len(words) != 4:
            warning.PCRFormatItemError("3", "24 - Isy=-1  Symm alpha", str(words))
        # init object
        ...
        for dp in xrange(1, DepMat+1):
            ...
            words = self.SplitNewLine(index)
            if len(words) != 5:
                warning.PCRFormatItemError("3", "24 - Isy=-1  Mag alpha", str(words))
            # init object
    ...
    def Line36ReadLaueClass(self, fit, sizemodel, index, LineItemList):
        ...
        for l in xrange(len(LineItemList)):
            ...
            if len(words1) != LineItemList[l]:
                #warning.PCRFormatItemError("3", "35 (3) - Line " + str(index) + " laue: " + laue.Name, str(words1))
                warning.PCRFormatItemError("3", "35 (3) - Line " + str(index) + " laue: ", str(words1))
            index = index + 1
            words2 = self.SplitNewLine(index)
            if len(words2) != LineItemList[l]:
                #warning.PCRFormatItemError("3", "35 (3) - Line " + str(index) + " laue: " + laue.Name, str(words2))
                warning.PCRFormatItemError("3", "35 (3) - Line " + str(index) + " laue: ", str(words2))
            for r in xrange(LineItemList[l]):
                # init
    ...
    def Line37ReadLaueClass(self, fit, strainmodel, index, LineItemList):
        ...
        if not isinstance(strainmodel, ContributionModule.StrainModelAnisotropic):
            raise NotImplementedError, "Line 37 Incorrect Type: " + strainmodel.__class__.__name__
        # read
        count = 0
    ...
    def Line43ReadFurth(self, fit, phase, index):
        ...
        for line in xrange(0, phase.get("Furth")):
            ...
            words = self.SplitNewLine(index)
            if len(words) != 3:
                warning.PCRFormatItemError("3", "Line 43 - " + str(line), str(words))
            namep  = words[0]
    ...
    def Line45ReadDis(self, fit, phase, index):
        ...
        for line in xrange(0, phase.get("Dis")):
            ...
            words = self.SplitNewLine(index)
            if len(words) != 8:
                warning.PCRFormatItemError("3", "Line 45 - " + str(line), str(words))
    ...
    def Line45_1ReadMomAngle(self, fit, phase, index):
        ...
        for line in xrange(1, phase.MomAngles+1):
            ...
            words = self.SplitNewLine(index)
            if len(words) != 13:
                warning.PCRFormatItemError("3", "Line 45-1 - " + str(line), str(words))
            soft.set("CATOD1", (words[0]))
            soft.set("CATOD2", (words[1]))
    ...
    def Line46ReadMomMoment(self, phase, fit, index):
        ...
        for line in xrange(1, phase.MomMoment+1):
            ...
            words = self.SplitNewLine(index)
            if len(words) != 3:
                warning.PCRFormatItemError("3", "Line 46 - " + str(line), str(words))
            soft.CATOD1 = (words[0])
...
class TimeReversalLookupTable:
    ...
    def lookUpNS(spacegroup):
        ...
        if spacegroup == "P 6/m m m":
            ...
            raise NotImplementedError("TimeReversalLookupTable lookUpNS cannot find " + spacegroup)
